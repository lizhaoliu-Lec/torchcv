...
class CosineLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, s=30.0, m=0.40):
        super(CosineLinear, self).__init__(in_features, out_features, False)
        self.s = s
        self.m = m
...
class SphereLinear(nn.Linear):
    ...
    def forward(self, input, label):
        ...
        if not self.training:
            return cos_theta * NormOfFeature.view(-1, 1)
...
LINEAR_DICT = {
    'linear': Linear,
    'arc_linear': ArcLinear,
    'cosine_linear': CosineLinear,
    'sphere_linear': SphereLinear,
    'nobias_linear': NobiasLinear
}
